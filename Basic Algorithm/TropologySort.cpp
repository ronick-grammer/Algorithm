...
void tropologySort(){ // 위상 정렬
    ...
    while(!q.empty()){ // 큐가 빌때까지 반복
        ...
        for(int i = 0; i < v[index].size(); i++){ // 연결된 노드의 수만큼 반복
            ...
            degree[x]--; // 해당 노드의 진입차수를 하나씩 감소시킨다
            if(degree[x] <= 0){  // 진입 차수가 0 이거나 보다 작으면
                q.push(x);       // 큐에 넣고
...
int main(void){
    ...
    for(int i = 1; i <= m; i++){
        ...
        cin >> from >> to;
        v[from].push_back(to);
        degree[to]++; // 해당 노드의 진입차수를 하나씩 증가시킨다
    }

    for(int i = 1; i <= n; i++){
        if(degree[i] == 0){ // 진입차수가 0인 노드들을 시작으로한다
            q.push(i);
            sorted.push_back(i);
        }
    }
